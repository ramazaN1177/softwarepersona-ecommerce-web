        ...
        <div className="bg-[#faf7f2] border border-[#e5dac8] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl space-y-4 p-6">
          ...
          <div className="flex items-start justify-between">
            <div className="flex items-center space-x-3">
              <div className="p-3 bg-[#f4ebe1] text-[#6f4e37] rounded-2xl border border-[#e5dac8]">
                <RefreshCw className="h-6 w-6" />
              </div>
              <div>
                <h3 className="text-base font-bold text-[#3d2b1f]">
                  {language === 'tr' ? 'Varsayılan Verilere Sıfırla' : 'Reset to Default Data'}
                </h3>
                <p className="text-xs text-[#785942]">
                  {language === 'tr' ? 'Örnek kitap listesine dönülsün mü?' : 'Restore sample book catalog?'}
                </p>
              </div>
            </div>
            <button
          ...
          <div className="bg-white p-4 rounded-xl border border-[#e8dfd1] text-xs text-[#543d2b] space-y-2">
            <p>
              {language === 'tr' 
                ? 'Tüm eklenen veya düzenlenen kitaplar sıfırlanıp varsayılan kitap listesine dönülecektir.' 
                : 'All added or edited books will be reset back to the default sample list.'}
            </p>
          </div>
          ...
          <div className="flex items-center justify-end space-x-3 pt-2">
            ...
            <button
              ...
              className="px-4 py-2 text-xs font-bold text-white bg-gradient-to-r from-[#6f4e37] to-[#8b5e34] hover:from-[#5a3e2b] rounded-xl shadow-md transition"
            >
              {language === 'tr' ? 'Evet, Sıfırla' : 'Yes, Reset'}
            </button>
          </div>
