        ...
        <div className="bg-[#faf7f2] border border-[#e5dac8] rounded-2xl w-full max-w-2xl overflow-hidden shadow-2xl my-8">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-[#e8dfd1] bg-[#f4ebe1]">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-lg font-bold text-[#3d2b1f]">
                {isEditing ? t('modal_edit_title') : t('modal_add_title')}
              </h3>
            </div>
            <button
              onClick={handleClose}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Kitap Adı */}
              <div>
                <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('label_book_title')} *</label>
                <input
                  type="text"
                  required
              ...
              {/* Yazar */}
              <div>
                <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('label_author')} *</label>
                <input
                  type="text"
                  required
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {/* Fiyat */}
              <div>
                <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('label_price')} *</label>
                <input
                  type="number"
                  required
                  min="1"
              ...
              {/* Stok */}
              <div>
                <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('label_stock')} *</label>
                <input
                  type="number"
                  required
                  min="0"
              ...
              {/* Kategori */}
              <div>
                <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('label_category')}</label>
                <select
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-bold text-[#543d2b] uppercase">{t('label_cover')}</label>
              ...
              {uploadType === 'file' ? (
                <div className="relative">
                  <label className="flex flex-col items-center justify-center w-full h-28 border-2 border-dashed border-[#d8cbb7] hover:border-[#8b5e34] rounded-xl cursor-pointer bg-white transition p-4 text-center">
                    ...
                    <span className="text-xs font-semibold text-[#543d2b]">{t('btn_file_upload')}</span>
            ...
            {formData.coverImage && (
              <div className="flex items-center justify-between p-2.5 bg-[#eaf3ed] rounded-xl border border-[#cce3d3]">
                <div className="flex items-center space-x-3">
                  <img src={formData.coverImage} alt="Önizleme" className="w-12 h-16 object-contain rounded-lg shadow-sm border border-[#b2d8bc]" />
                  <div>
                    <span className="text-xs text-[#2e6f40] font-bold block">Resim Hazır</span>
            ...
            {/* Açıklama */}
            <div>
              <label className="block text-xs font-bold text-[#543d2b] uppercase mb-1">{t('summary_title')}</label>
              <textarea
                rows={3}
            ...
            <div className="pt-4 border-t border-[#e8dfd1] flex justify-end space-x-3">
              ...
              >
                {isEditing ? t('btn_save') : t('btn_add')}
              </button>
            </div>
