  ...
  if (!book) {
    return (
      <div className="text-center py-20 bg-white border border-[#e8dfd1] rounded-2xl my-8">
        <h3 className="text-xl font-bold text-[#3d2b1f] mb-2">{language === 'tr' ? 'Kitap Bulunamadı' : 'Book Not Found'}</h3>
        <p className="text-sm text-[#785942] mb-6">{language === 'tr' ? 'Aradığınız kitap sistemde kayıtlı değil veya silinmiş olabilir.' : 'The requested book is not found in system or might have been deleted.'}</p>
        <Link
          to="/books"
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          ...
          <div className="md:col-span-7 flex flex-col justify-between space-y-6">
            <div>
              ...
              <p className="text-base font-semibold text-[#785942] mt-1">{t('label_author')}: {book.author}</p>

              {/* Fiyat ve Puan Rozet Paneli */}
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 my-6">
                <div className="p-3.5 bg-[#faf7f2] rounded-xl border border-[#e8dfd1]">
                  <span className="text-[11px] font-semibold text-[#785942] block">{t('table_price')}</span>
                  <span className="text-2xl font-extrabold text-[#6f4e37]">{book.price} ₺</span>
                </div>
                <div className="p-3.5 bg-[#faf7f2] rounded-xl border border-[#e8dfd1]">
                  <span className="text-[11px] font-semibold text-[#785942] block">{language === 'tr' ? 'Değerlendirme' : 'Rating'}</span>
                ...
                <div className="p-3.5 bg-[#faf7f2] rounded-xl border border-[#e8dfd1]">
                  <span className="text-[11px] font-semibold text-[#785942] block">{language === 'tr' ? 'Sayfa Sayısı' : 'Page Count'}</span>
                  <span className="text-lg font-bold text-[#3d2b1f]">{book.pages || 320}</span>
                </div>
              </div>
              ...
              {viewMode === 'customer' && (
                <div className="p-5 bg-[#faf7f2] rounded-2xl border border-[#e8dfd1] space-y-4 my-6">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-[#3d2b1f] uppercase tracking-wider">{t('order_quantity')}</span>
                    <span className="text-xs text-[#785942]">{t('max_stock_limit')}: {book.stock}</span>
                  </div>
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 items-center">
                    ...
                    <div className="text-right sm:text-left text-xs font-bold text-[#6f4e37] bg-white px-4 py-2.5 rounded-xl border border-[#e5dac8] w-full sm:w-auto text-center">
                      {t('cart_total')}: <span className="text-base font-extrabold">{book.price * quantity} ₺</span>
                    </div>
                  </div>
              ...
              <div className="space-y-2">
                <h3 className="text-sm font-bold text-[#3d2b1f] uppercase tracking-wider">{t('summary_title')}</h3>
                <p className="text-sm text-[#543d2b] leading-relaxed bg-[#faf7f2] p-4 rounded-xl border border-[#f2ebdc]">
                  {book.description || (language === 'tr' ? 'Bu kitap için detaylı açıklama metni girilmemiştir.' : 'No detailed summary provided for this book.')}
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-[#f2ebdc] grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs text-[#785942]">
              <div className="flex items-center space-x-2 bg-[#faf7f2] p-2.5 rounded-xl border border-[#f2ebdc]">
                ...
                <span className="font-semibold text-[11px]">{t('shipping_badge_24h')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-[#faf7f2] p-2.5 rounded-xl border border-[#f2ebdc]">
                <ShieldCheck className="h-4 w-4 text-[#2e6f40]" />
                <span className="font-semibold text-[11px]">{t('original_badge')}</span>
              </div>
              <div className="flex items-center space-x-2 bg-[#faf7f2] p-2.5 rounded-xl border border-[#f2ebdc]">
                <PackageCheck className="h-4 w-4 text-[#8b5e34]" />
                <span className="font-semibold text-[11px]">{t('packaging_badge')}</span>
              </div>
            </div>
